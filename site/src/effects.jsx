...
/* Shared effects + tiny hooks (ES module port of the new design). */
...
export function CosmicCanvas({ density = 1 }) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (const s of stars) {
        ...
        ctx.beginPath()
        ctx.fillStyle = s.purple ? `rgba(192,132,252,${a})` : `rgba(255,255,255,${a})`
...
/* Count-up number */
export function CountUp({ to, duration = 1800, suffix = '', prefix = '' }) {
...
export function Typewriter({ lines, speed = 30, lineDelay = 220, onDone }) {
  ...
  }, [ci, li, lineDelay, lines, onDone, speed])
