// Developer Core System scene — replaces the static orb.
// Interactive: hover any tech node → glow + connector line to core; speed varies per ring.
const { useEffect, useRef, useState: useStateCS } = React;

function CoreSystem({ size = 520, mouseX = 0, mouseY = 0 }) {
  const [hovered, setHovered] = useStateCS(null);
  const [tick, setTick] = useStateCS(0);
  ...
  // 3 rings, different speeds, different tilts
  const rings = [
  ...
  return (
    <div className="core-wrap" style={{ width: size, height: size, transform: `rotateY(${tx}deg) rotateX(${ty}deg)` }}>
      ...
      <div className="core-hud core-hud-br">
        <div className="hud-row dim" style={{textAlign: 'right'}}>{hovered ? `→ ${placed.find(p => p.id === hovered)?.label.toUpperCase()}` : 'IDLE'}</div>
      </div>
      ...
      <svg className="core-svg" viewBox={`-${size/2} -${size/2} ${size} ${size}`}>
        ...
        {/* Connector line from hovered node → core */}
        {hovered && (() => {
      ...
      {/* Tech node chips, positioned absolutely over the SVG */}
      {placed.map((n) => (
        <CoreNode
          key={n.id}
          {...n}
...
function CoreNode({ x, y, label, id, active, onEnter, onLeave }) {
  return (
    <button
      type="button"
